...
完整性分數是用來判斷購買群組是否擁有指派給所需角色的正確成員數量，以及是否準備好用於帳戶歷程中。 此分數是根據購買群組中的角色數量，以及每個已定義角色的完整度的百分比。

當您建立購買群組時，就會開始計算初始完整度分數，並且每天都會重新計算一次，每次建立或更新購買群組時，都會重新計算初始完整度分數。
