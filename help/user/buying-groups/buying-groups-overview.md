...
>[!NOTE]
>
>從 2025.10 版本開始，每日都會執行自動化購買群組維護工作。
...
我們使用完整性分數來確認此購買群組是否已將正確的成員人數指派至所需角色，並且已準備好供帳戶歷程使用。此分數是一個百分比，以購買群組中的角色數量以及每個已定義角色的完整性為基礎。

初始的完整性分數會在您建立購買群組後立即開始計算，而且每一天以及每次建立或更新購買群組時都會重新計算。
