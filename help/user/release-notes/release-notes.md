...
| 功能 | 體驗事件歷史記錄篩選 | 此功能可讓B2B歷程協調使用歷史參與度，推動受眾成員的行為型決策。 |
| 功能 | 歷程分割路徑決策 | 您可以針對帳戶和人員歷程最佳化分割路徑決策，以精簡在歷程啟用前或執行階段中對象在路徑間的路由方式。 |
| 增強功能 | Audience Agent技能 | 更新Audience Agent技能，包括AI支援的購買群組範本建立。 使用第一方意圖和角色對應來產生購買群組範本、檢閱AI建議的角色對角色對角色對角色對應，以及在發佈範本之前以自然語言調整範本。 |
| 增強功能 | 電子郵件參與績效報表 | 「電子郵件參與效能報表」為行銷人員提供了在Adobe Journey Optimizer B2B edition中跨所有歷程的電子郵件活動的統一檢視。 |
| 增強功能 | 個人歷程總覽儀表板 | 此儀表板提供您作用中人員歷程的完整概觀。 |
