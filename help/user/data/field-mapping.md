...
>[!TIP]
>
>您可以使用XDM商業帳戶個人關係類別(如[Experience Platform XDM檔案](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/xdm/tutorials/relationship-b2b)所述)，以多對多關係來建立XDM商業帳戶和XDM商業帳戶類別的模型。
...
|------------------- |---------------------------------- |--------------------------- |-------- |--------------- |
| `personRoles` | 人員角色 | 角色 | 字串陣列 | 與帳戶中的個人相關聯的角色陣列，例如`owner, accountant, designer`。 |
...
| `consents.marketing.email.reason` | 選擇不接收的原因 | 取消訂閱原因 | 字串 | 與电子郵件選擇退出關聯的原因。 |
...
| `person.name.firstName` | 名字 | 名字 | 字串 | 在姓名的語言中最常被接受的書寫順序的第一個姓名區段。 在許多文化中，它是首選的人名或名字。 引入了 lastName 和 lastName 屬性，以保持與以簡化、非語義和不可國際化方式對名稱建模的現有系統的相容性。 使用 `xdm:fullName` 總是更可取的。 |
| `person.name.lastName` | 姓氏 | 姓氏 | 字串 | 名稱在書面順序中的最後區段，在名稱語言中最普遍接受。 在許多文化中，這是繼承的姓氏、父名或母名。 FirstName和lastName屬性的引入是為了與現有系統保持相容性，現有系統以簡化、非語意和無法國際化的方式模型化名稱。 使用`xdm:fullName`永遠是較好的。 |
...
| [財產](https://github.com/adobe/xdm/blob/master/docs/reference/mixins/account/account-details.schema.md) | 顯示名稱 | Journey Optimizer B2B顯示名稱 | 資料類型 | 說明 |
|------------------- |---------------------------------- |--------------------------- |-------- |--------------- |
| `accountBillingAddress.city` | 城市 | 城市 | 字串 | 帳單地址中使用的城市名稱。 |
| `accountBillingAddress.country` | 國家/地區 | 國家/地區 | 字串 | 帳單地址中使用的政府管理區域名稱。 除 之外 `xdm:countryCode`，它是一個免費格式的字段，可以使用任何語言的國家/地區名稱。 |
| `accountBillingAddress.postalCode` | 郵遞區號 | 地址郵政Code | 字串 | 帳單地址所在位置的郵遞區號。 郵遞區編碼並非適用於所有國家/地區。 在某些國家/地區，它僅包含部分郵遞區號。 |
...
| `accountName` | 名稱 | 名稱 | 字串 | **必填欄位** <br/>公司名稱。 此欄位中最多允許 255 個字元。 |
...
此外，機會資料會儲存為XDM商業機會類別中的屬性，可透過多對一關係與XDM商業帳戶類別建立關聯，如[這裡](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/xdm/tutorials/relationship-b2b#relationship-field)所述。

| [屬性](https://github.com/adobe/xdm/blob/master/docs/reference/adobe/experience/marketo/opportunity-marketo.schema.md) | 顯示名稱 | Journey Optimizer B2B顯示名稱 | 資料類型 | 說明 |
|------------------- |---------------------------------- |--------------------------- |-------- |--------------- |
| `expectedCloseDate` | 預計關閉日期 | 預期機會關閉日期 | 字串 | 商機預計關閉日期。 |
| `expectedRevenue.amount` | 預期收入 | 預計商機收入總量 | 字串 | 根據金額和機率計算的收入。 |
| `fiscalQuarter` | 會計季度 | 機會會計季度 | 字串 | 機會的目標會計季度。 |
| `fiscalYear` | 會計年度 | 機會會計年度 | 字串 | 機會的目標會計年度。 |
| `forecastCategory` | 預測類別 | 機會預測類別 | 字串 | 預測類別由商機階段值確定。 |
...
| `opportunityAmount.amount` | 機會金額 | 機會數量總計 | 字串 | 該商機的預估總銷售金額。 |
...
| `opportunityStage` | 機會階段 | 機會階段 | 字串 | 商機的銷售階段，以幫助銷售團隊努力贏得它。 |
| `opportunityType` | 機會類型 | 機會類型 | 字串 | 指派給機會的型別，例如&#x200B;_現有企業_&#x200B;或&#x200B;_新企業_ |
