...
Adobe Journey Optimizer B2B edition利用附加的Marketo Engage例項中的電子郵件通道功能和事件追蹤。 為確保電子郵件傳送如預期運作，適用於使用限制防火牆或Proxy伺服器設定的組織，系統管理員必須將特定網域和IP位址範圍新增至允許清單。

>[!NOTE]
>
>如果您的組織已使用連線的Marketo Engage執行個體來執行行銷作業，這些通訊協定和設定應該已準備就緒。
...
請完成下列步驟，以確保追蹤和電子郵件傳遞：

1. [建立登入頁面和電子郵件的DNS記錄](#create-dns-records-for-landing-pages-and-email)
...
>[!NOTE]
>
>電子郵件傳遞服務與諮詢是Adobe的個別付費方案。 如果您需要或想要Journey Optimizer B2B edition執行個體的傳遞團隊的支援，必須購買該執行個體的其中一個電子郵件傳遞服務套件（Essentials、Enhanced或Plus）。 這與預先存在的Marketo Engage例項上的任何傳遞能力套件無關。 傳遞能力服務是按執行個體附加的，而非按組織附加的。 兩個執行個體上的傳遞能力支援需要兩個不同的傳遞能力服務套件。 每當為Journey Optimizer B2B edition布建新IP時，都需要新的傳遞服務套件才能進行IP預熱和持續傳遞支援。
...
身為管理員，您應該與行銷團隊合作，規劃及實作兩個CNAME記錄。 第一個是用於登入頁面URL，讓登入頁面以可反映您網域的URL顯示，而非以Adobe Marketo Engage （實際主機）顯示。 第二個用於透過Marketo Engage傳送的電子郵件中所包含的追蹤連結。

### 新增登入頁面的CNAME

將登入頁面CNAME新增至您的DNS記錄，這樣`[YourLandingPageCNAME]`會指向指派給登入頁面的唯一帳戶字串。 登入您的網域註冊機構的網站，然後輸入登陸頁面CNAME和帳戶字串。 此專案通常涉及三個欄位：

* 別名：輸入`[YourLandingPageCNAME]`
* 類型：CNAME
* 指向：輸入`[MunchkinID].mktoweb.com`
...
您可以對生產DKIM執行個體和附加的Marketo Engage B2B edition執行個體使用相同的Journey Optimizer設定。 在附加的執行個體中，建立與您的Marketo Engage執行個體中完全相同的網域。 選擇器和加密值不需要相符。 將網域新增到Journey Optimizer B2B edition執行個體後，請開啟Adobe支援票證以請求將您的DKIM設定從Marketo Engage執行個體分享到新執行個體。 提供您的Marketo Engage首碼(Munchkin ID)和新的Journey Optimizer B2B edition首碼(Munchkin ID)。
...
* 有效的 SPF
* 您的FROM：網域有效的DKIM記錄（建議用於[!DNL Marketo Engage]和[!UICONTROL Journey Optimizer B2B edition]）

此外，請為您的`FROM:`網域設定DMARC專屬的DNS TXT記錄。 或者，您可以定義電子郵件，指定 DMARC 報告應該發送到組織的哪個單位進行報告監控。

### DMARC 工作流程範例

>[!TIP]
>
>最佳實務是以&#x200B;_逐步轉出_&#x200B;的形式實作DMARC。 當您了解潛在影響後，請將您的 DMARC 原則從 `p=none` 升級到 `p=quarantine`，然後再升級到 `p=reject`，並將您的 DMARC 原則設定為對 SPF 和 DKIM 採寬鬆比對。
...
DMARC 可以接收未通過 SPF/DKIM 驗證的電子郵件之報告。 在驗證程式中，ISP服務會產生兩個不同的報告。 寄件者可以透過其 DMARC 原則中的 RUA/RUF 標記接收這些報告。

* **彙總報告 (RUA)**：不包含任何可能屬於 GDPR (一般資料保護規範) 敏感資訊的個人識別資訊 (PII)。

* **鑑識報告 (RUF)**  - 包含屬於 GDPR 敏感資訊的電子郵件。 在實施此報告之前，請先確認您的組織處理必須符合 GDPR 規範的資訊時採取什麼原則。

這些報告的主要用途是讓您大致了解有哪些偽冒的電子郵件。 它們是高度技術性的報告，最好透過協力廠商工具進行分析。
...
如需關於 DMARC 及其所有選項的詳細資訊，請參閱 [https://dmarc.org/](https://dmarc.org/){target="_blank"}。
...
Marketo Engage會代表您對網際網路伺服器進行輸出連線。 您的IT組織和某些合作夥伴/廠商可能會使用允許清單來限制對伺服器的存取。 若是如此，您必須提供Marketo Engage輸出IP位址區塊，以新增至其允許清單。
