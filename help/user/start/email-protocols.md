...
>[!NOTE]
>
>電子郵件傳遞服務與諮詢是Adobe的個別付費方案。 如果您需要或想要Journey Optimizer B2B edition執行個體的傳遞團隊的支援，必須購買該執行個體的其中一個電子郵件傳遞服務套件（Essentials、Enhanced或Plus）。 此設定與預先存在的Marketo Engage例項上的任何傳遞能力套件無關。 傳遞能力服務是按執行個體附加的，而非按組織附加的。 兩個執行個體上的傳遞能力支援需要兩個不同的傳遞能力服務套件。 每當為Journey Optimizer B2B edition布建新IP時，都需要新的傳遞服務套件才能進行IP預熱和持續傳遞支援。

## 建立登入頁面和電子郵件的DNS記錄

透過連接 CNAME 記錄，行銷人員可以託管擁有一致品牌形象的電子郵件、登陸頁面和部落格的網頁版，從而提高流量和轉換率。 強烈建議您將 CNAME 新增至根網域主機，以便 Marketo Engage 託管以行銷為主的 Web 資產。 若要針對透過Marketo Engage傳送的電子郵件中所包含的追蹤連結，規劃及實作CNAME記錄，請以管理員身分與您的行銷團隊合作。
...
您的行銷團隊應提供 DKIM (Domain Keys Identified Mail) 資訊以新增至您的 DNS 資源記錄。 若要設定DKIM和SPF (Sender Policy Framework)，請按照以下步驟操作，然後在更新時通知您的行銷團隊。

您可以對生產DKIM執行個體和附加的Marketo Engage B2B edition執行個體使用相同的Journey Optimizer設定。 在附加的執行個體中，建立與您的Marketo Engage執行個體完全相同的網域。 選擇器和加密值不需要相符。 將網域新增到Journey Optimizer B2B edition執行個體後，請開啟Adobe支援票證以請求將您的DKIM設定從Marketo Engage執行個體分享到新執行個體。 提供您的Marketo Engage首碼(Munchkin ID)和新的Journey Optimizer B2B edition首碼(Munchkin ID)。
...
此外，請為您的`FROM:`網域設定DMARC專屬的DNS TXT記錄。 或者，您可以定義電子郵件地址，指定DMARC報告在組織內的哪個位置用來監控報告。
...
這些報告的主要用途是接收嘗試詐騙的電子郵件概觀。 它們是高度技術性的報告，最好透過協力廠商工具進行分析。
