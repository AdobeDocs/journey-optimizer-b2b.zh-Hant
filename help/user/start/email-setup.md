---
title: 電子郵件設定
description: 設定Journey Optimizer B2B電子郵件傳送的Marketo Engage選項，包括預設值、取消訂閱、網頁檢視、Velocity物件限制、追蹤標題和機器人篩選。
...
若要為附加的Marketo Engage執行個體設定電子郵件預設值，請變更設定值以反映行銷團隊的使用情況。
...
對於非營運行銷電子郵件，取消訂閱文字和連結會附加在底部。 作為產品管理員，請設定預設HTML，以及行銷人員將電子郵件標示為運作時填入的文字。
...
電子郵件內容的顯示功能有限（CSS有限，無JavaScript或表單）。 行銷人員可使用&#x200B;_以網頁形式檢視_&#x200B;選項，透過Marketo Munchkin為電子郵件收件者套用Cookie。 作為產品管理員，請設定預設的HTML，以及行銷人員選擇此選項時填入的文字。
...
1. 捲動至&#x200B;_[!UICONTROL 自訂物件擷取限制]_&#x200B;面板，並在&#x200B;**[!UICONTROL 父擷取限制中輸入新值]**
欄位。

   ![Marketo Engage電子郵件管理員 — 自訂物件擷取限制預設值](./assets/me-admin-email-custom-object-retrieval-limits.png){width="500"}

   支援介於10到100之間的值。 系統會自動設定&#x200B;_[!UICONTROL 子擷取限制]_，將1000除以父項限制。 例如，如果您將父限制設為50，子限制的計算方式為20 (1000 ÷ 50 = 20)。

1. 按一下&#x200B;**[!UICONTROL 儲存變更]**。

## 自訂標頭選項

變更電子郵件的&#x200B;_[!UICONTROL 自訂標題選項]_&#x200B;以設定電子郵件追蹤連結標題。 啟用這些選項，以使用具有嚴格傳輸的HTTPS來實作安全追蹤連結。

1. 移至附加的Marketo Engage執行個體中的&#x200B;**[!UICONTROL 管理員]**&#x200B;區域，並選取&#x200B;**[!UICONTROL 電子郵件]**。

1. 捲動至&#x200B;_[!UICONTROL 自訂標題選項]_&#x200B;面板，並根據您的追蹤連結原則變更設定：

   ![Marketo Engage電子郵件管理員 — 自訂標頭選項預設設定](./assets/me-admin-email-custom-header-options.png){width="500"}

   * **[!UICONTROL 嚴格傳輸安全性]** — 將此選項設定為&#x200B;_已啟用_，以確保追蹤連結一律透過HTTPS提供。 僅對追蹤連結由SSL保護的訂閱啟用此功能。
   * **[!UICONTROL Max-age]** — 此欄位支援必要指示詞，以秒為單位指定瀏覽器記憶僅透過HTTPS存取網域的時間。
...
1. 切換滑桿以啟用其中一項或兩項。
   ...
   >[!NOTE]
   >
   >當&#x200B;_活動_&#x200B;之間的持續時間`0`秒設定時，Marketo Engage會識別在該秒發生的電子郵件活動。 如果在指定的秒數內發生多個電子郵件活動，則會將其識別為機器人活動。

   若要停用任一篩選方法，請將滑桿切換至左側。 如果您這麼做，資料不會重設。

### IP封鎖清單

Adobe已找出負責產生假參與的IP位址。 系統會自動篩選這些IP的參與，並將其從您的Marketo Engage執行個體中排除。 此篩選可能會減少電子郵件開啟、點按及其他相關活動。 此清單可能會定期更新。
...
>[!NOTE]
>
>每個IP位址都經過仔細的分析和檢查，才列入此清單，確保只封鎖最關鍵和最有害的IP。
