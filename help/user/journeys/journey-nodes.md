...
* [帳戶客群](#account-audience-node)
* [採取動作](#take-an-action)
* [接聽事件](#listen-for-an-event)
* [分割路徑](#split-paths)
* [等待](#wait)
* [合併路徑](#merge-paths)
...
| 節點內容 | 動作 | 限制 |
| ------------ | ------ | ----------- |
| [人員](#add-a-people-action) | 新增至清單 | 選取Marketo Engage工作區<br/>清單名稱 |
| | 新增至Marketo Engage請求行銷活動 | 選取Marketo Engage工作區<br/>選取要求行銷活動 |
| | 指派給購買群組 | 選取方案興趣<br/>選取角色 |
| | 變更Marketo Engage中的人員分割 | 新磁碟分割 |
| | 變更分數 | 分數名稱<br/>變更 |
| | 個人有趣的時刻 | 型別<br/>描述 |
| | 從購買群組移除 | 選取解決方案興趣 |
| | 從清單移除 | 選取Marketo Engage工作區<br/>清單名稱 |
| | 傳送電子郵件 | 建立新電子郵件<br/>從Marketo Engage中選取電子郵件 |
| | 傳送簡訊 | 建立簡訊 |
| [帳戶](#add-an-account-action) | 帳戶變更資料值 | 選取屬性<br/>新值 |
| | 帳戶有趣的時刻 | 型別（電子郵件、里程碑或網頁）<br/>說明（選擇性） |
| | 將帳戶新增至（其他）歷程 | 選取即時帳戶歷程 |
| | 從歷程移除帳戶 | 選取即時帳戶歷程 |
| | 傳送銷售警報 | 選取感興趣的解決方案<br/>傳送電子郵件至 |
| | 更新購買群組狀態 | 選取方案興趣<br/>狀態（必要，最多50個字元） |
...
| 節點內容 | 活動 | 限制 |
| ------------ | ----- | ----------- |
| [人員](#add-a-people-event) | 已指派給購買群組 | 方案興趣<br/>其他限制（選擇性）： <ul><li>角色</li><li>活動日期</li></ul><br/>逾時（選擇性） |
| | 電子郵件中的點按連結 | 電子郵件<br/>其他限制（選擇性）： <ul><li>連結</li><li>連結識別碼</li><li>是行動裝置</li><li>裝置</li><li>平台</li><li>瀏覽器</li><li>為預測性內容</li><li>是機器人活動</li><li>機器人活動模式</li><li>瀏覽器</li><li>活動日期</li><li>最低 次數</li></ul><br/>逾時（選擇性） |
| | 簡訊中的點按連結 | 電子郵件<br/>其他限制（選擇性）：<ul><li>連結</li><li>裝置</li><li>平台</li><li>活動日期</li><li>最低 次數</li></ul><br/>逾時（選擇性） |
| | 資料值變更 | 個人屬性<br/>其他限制（選擇性）：<ul><li>新值</li><li>上一個值</li><li>原因</li><li>來源</li><li>活動日期</li><li>最低 次數</li></ul><br/>逾時（選擇性） |
| | 開啟電子郵件 | 電子郵件<br/>其他限制（選擇性）： <ul><li>連結</li><li>連結識別碼</li><li>是行動裝置</li><li>裝置</li><li>平台</li><li>瀏覽器</li><li>為預測性內容</li><li>是機器人活動</li><li>機器人活動模式</li><li>瀏覽器</li><li>活動日期</li><li>最低 次數</li></ul><br/>逾時（選擇性） |
| | 已從購買群組移除 | 方案興趣<br/>活動日期（選擇性）<br/>逾時（選擇性） |
| | 分數已變更 | 分數名稱<br/>其他限制（選擇性）：<ul><li>變更</li><li>新分數</li><li>急迫性</li><li>優先等級</li><li>相對分數</li><li>相對急迫性</li><li>活動日期</li><li>最低 次數</li></ul><br/>逾時（選擇性） |
| | 簡訊退信 | SMS訊息<br/>其他限制（選擇性）：<ul><li>活動日期</li><li>最小次數</li></ul><br/>逾時（選擇性） |
| [帳戶](#add-an-account-event) | 帳戶有一個有趣的時刻 | 型別（電子郵件、里程碑或Web）<br/>其他限制（選擇性）：<ul><li>說明</li><li>來源</li><li>活動日期</li></ul> <br/>逾時（選擇性） |
| | 帳戶資料值的變更 | 屬性<br/>其他限制（選擇性）：<ul><li>新值</li><li>上一個值</li><li>活動日期</li></ul> <br/>逾時（選擇性） |
| | 購買群組狀態的變更 | 方案興趣<br/>其他限制（選擇性）：<ul><li>新狀態</li><li>先前的狀態</li><li>活動日期</li></ul><br/>逾時（選擇性） |
| | 完整度分數的變更 | 方案興趣<br/>其他限制（選擇性）：<ul><li>新分數</li><li>前一個分數</li><li>活動日期</li></ul><br/>逾時（選擇性） |
| | 參與分數的變更 | 方案興趣<br/>其他限制（選擇性）：<ul><li>新分數</li><li>前一個分數</li><li>活動日期</li></ul><br/>逾時（選擇性） |
...
| 節點內容 | 路徑條件 | 說明 |
| ------------ | --------------- | ----------- |
| [人員](#add-a-split-path-by-people-node) | [!UICONTROL 個人屬性] | 個人設定檔中的屬性，包括： <ul><li>城市</li><li>國家/地區</li><li>出生日期</li><li>電子郵件地址</li><li>電子郵件無效</li><li>電子郵件已暫停</li><li>名字</li><li>推斷的狀態區域</li><li>職稱</li><li>姓氏</li><li>行動電話號碼</li><li>電話號碼</li><li>郵遞區號</li><li>狀態</li><li>退訂</li><li>取消訂閱的原因</li></ul> |
| | [!UICONTROL 活動歷史記錄] > [!UICONTROL 電子郵件] | 與歷程相關聯的電子郵件活動： <ul><li>[!UICONTROL 已點按電子郵件中的連結]</li><li>已開啟的電子郵件</li><li>已傳遞電子郵件</li><li>已傳送電子郵件</li></ul> 會使用歷程中先前選取的電子郵件訊息評估這些條件。 |
| | [!UICONTROL 活動歷史記錄] > [!UICONTROL 資料值已變更] | 針對選取的人員屬性，發生值變更。 這些變更型別包括： <ul><li>新值</li><li>上一個值</li><li>原因</li><li>來源</li><li>活動日期</li><li>最低 次數</li></ul> |
| | [!UICONTROL 活動歷史記錄] > [!UICONTROL 有趣的時刻] | 在相關Marketo Engage例項中定義的有趣時刻活動。 限制包括： ul><li>里程碑</li><li>電子郵件</li><li>Web</li></ul> |
| | [!UICONTROL 特殊篩選器] > [!UICONTROL 購買團體成員] | 該人員是或不是根據下列一或多個條件評估的購買群組成員： <ul><li>解決方案興趣</li><li>購買群組狀態</li><li>完整度分數</li><li>參與分數</li><li>角色</li></ul> |
| [帳戶](#add-a-split-path-by-account-node) | 帳戶屬性 | 帳戶設定檔中的屬性，包括： <ul><li>年收入</li><li>城市</li><li>國家/地區</li><li>員工人數</li><li>行業</li><li>名稱</li><li>SIC代碼</li><li>狀態</li></ul> |
| | [!UICONTROL 特殊篩選器] > [!UICONTROL 有購買群組] | 此帳戶是否具有根據下列一或多個條件評估的購買群組成員： <ul><li>解決方案興趣</li><li>購買群組狀態</li><li>完整度分數</li><li>參與分數</li></ul> |
