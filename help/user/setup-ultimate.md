...
Adobe Journey Optimizer B2B Edition是以Adobe Experience Platform為基礎。 透過此實作，Journey Optimizer B2B Edition和Marketo Engage不在相同的系統和相同的資料存放區中。 Journey Optimizer B2B Edition會從Adobe Experience Platform接收資料。 但是，它繼續仰賴Marketo Engage許可權和部分後端功能（例如電子郵件傳送）來布建和設定系統。
