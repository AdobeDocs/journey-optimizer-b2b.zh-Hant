...
Adobe Journey Optimizer B2B edition是以Adobe Experience Platform為建置基礎。 透過此實作，Journey Optimizer B2B edition和Marketo Engage不在相同的系統和相同的資料存放區中。 Journey Optimizer B2B edition會從Adobe Experience Platform接收資料。 但是，它繼續仰賴Marketo Engage許可權和部分後端功能（例如電子郵件傳送）來布建和設定系統。
...
此實施是解鎖Journey Optimizer B2B edition功能的基礎：

* **輕鬆統一並擴充您的資料：**&#x200B;平台支援複雜的資料模型，包括自訂物件、購買群組和帳戶事件。
