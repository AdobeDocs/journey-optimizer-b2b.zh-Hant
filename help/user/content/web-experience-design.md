...
>title="使用瀏覽模式"
>abstract="在此模式下，您可以導覽至您要為所選網頁管道設定進行個人化的確切頁面。"
...
| **[!UICONTROL 安排]** | 在顯示內將選取的元素向後或向前移動。 |
...
* **[!UICONTROL Image]** - Use this component to insert an image file into the page. You can adjust styling attributes such as the width and height from the properties in the right panel. 如需詳細資訊，請參閱&#x200B;_內容元件_&#x200B;中的[影像](./content-components.md#image)。
* **[!UICONTROL 標題]** — 使用此元件插入標題類別文字。 您可以從右側面板中的屬性調整樣式屬性，例如文字顏色、樣式、字型和大小。 如需詳細資訊，請參閱&#x200B;_內容元件_&#x200B;中的[文字](./content-components.md#text)。
* **[!UICONTROL 段落]** — 使用此元件插入標準文字元素。 您可以從右側面板中的屬性調整樣式屬性，例如文字顏色、樣式、字型和大小。 如需詳細資訊，請參閱&#x200B;_內容元件_&#x200B;中的[文字](./content-components.md#text)。
* **[!UICONTROL 連結]** — 使用此元件將獨立文字連結插入至指定的URL。 您可以從右側面板中的屬性調整樣式屬性，例如文字顏色、樣式、對齊方式和大小。

在左側選取元件型別，然後將滑鼠游標停留在您要新增元件的位置旁的元素上。

![視覺化編輯器介面 — 新元件](./assets/web-experience-design-visual-editor-insert-component.png){width="800" zoomable="yes"}

按一下顯示的按鈕之一來放置元件：

* ***[!UICONTROL 插入至]**&#x200B;之前 — 在選取的元素之前插入元件。
* ***[!UICONTROL 插入至]**&#x200B;之後 — 在選取的元素之後插入元件。

若要取消選取要插入的元件型別，請在頁面頂端顯示的內容藍色橫幅中按一下&#x200B;**[!UICONTROL ESC]**。

## 非視覺化編輯器 {#non-visual-editor}

當您需要進行無法在視覺化編輯器中輕鬆完成的修改時，請使用非視覺化編輯器。 這種程式碼型方法可讓您精確控制元素目標定位和修改。 完成下列步驟，使用非視覺化編輯器設計您的網頁體驗：

1. 在網頁體驗詳細資料頁面中顯示&#x200B;_[!UICONTROL 內容]_&#x200B;索引標籤時，按一下右側面板中的&#x200B;**[!UICONTROL 新增修改]**。
...
1. 定義您要進行的第一個修改。
   ...
   左側面板會顯示現有修改的清單（如果有的話）。 按一下&#x200B;**[!UICONTROL 新增]**&#x200B;以定義新的修改。 如果未定義任何修改，面板會預設為&#x200B;_[!UICONTROL 新增修改]_&#x200B;選項。
   ...
   * 選擇&#x200B;**[!UICONTROL 修改型別]**：
     ...
     | [**[!UICONTROL 頁面]**](#page-modifications) | 將自訂HTML、CSS或JavaScript插入頁面層級的元素，例如`<head>`或`<body>`。 |
...
1. （選用）按一下&#x200B;**[!UICONTROL 新增個人化]**，並使用[個人化編輯器](./personalization.md#personalization-editor)建立內容的自訂個人化。
...
1. 在&#x200B;**[!UICONTROL 內容]**&#x200B;方塊中新增您的自訂程式碼。
   ...
   >[!CAUTION]
   >
   >您只能將`<script>`和`<style>`專案新增至`<head>`區段。 新增`<div>`標籤和其他元素可能會造成剩餘`<head>`個元素填入`<body>`。
...
您建立的所有修改都會受到追蹤，並可從視覺化編輯器和非視覺化編輯器的&#x200B;**[!UICONTROL 修改]**&#x200B;面板進行管理。 按一下左側工具列中的&#x200B;_[!UICONTROL 修改]_ <!-- ( ![Modifications icon](../assets/do-not-localize/icon-web-exp-modifications.svg) ) -->圖示以檢視所有修改。
...
如需Customer Journey Analytics和建立網頁報表的詳細資訊，請參閱[Customer Journey Analytics檔案](https://experienceleague.adobe.com/zh-hant/docs/analytics-platform/using/cja-landing)。
...
1. 開啟左側面板中的點選追蹤清單，並修改&#x200B;**[!UICONTROL 追蹤的動作]**&#x200B;值，以在您的報告中識別此互動。
