...
description: Journey Optimizer B2B edition中的Audience Agent B2B使用意圖分析和角色對應來建立購買群組，並加速B2B行銷工作流程。
...
* **_利用AI來偵測並建立購買群組_**：使用AI、結構化、非結構化資料以及統一的第一方資料，以簡化購買群組探索與建立。

![Audience Agent B2B處於全頁模式](./assets/audience-agent-full.png){width="700" zoomable="yes"}

>[!PREREQUISITES]
>
>若要使用Audience Agent B2B，必須有必要的資料定義和對應： <br/>
>
...
| 購買群組的建立與行動 | <li>根據觀察到的角色和角色模式，建議角色對角色的對應。 <li>產生指定產品的購買群組角色範本。 <li>透過包含或排除特定角色和角色，支援範本自訂。 <li>在建立購買群組之前，先驗證是否已定義所需的角色。 | <li>減少手動操作，以及購買群組範本不完整的風險。 <li>確保在建立之前驗證角色涵蓋範圍，以降低涵蓋範圍差距的風險。 <li>將分析見解轉換為立即的作業性後續步驟。 |

## 限制

Audience Agent B2B取決於設定的意圖分類法、XDM欄位對應和體驗事件資料。 機會資料不完整、意圖分類遺失或過期，或所需的設定檔和帳戶識別碼未對應時，深入分析的可靠性會降低。 對於意圖計算，代理程式只會處理這些體驗事件： `directMarketing.emailClicked`、`directMarketing.emailOpened`、`directMarketing.emailUnsubscribed`和`web.webpagedetails.pageViews`。
...
* 顯示與`<product>`關聯的角色。
* 為我顯示`<product>`的角色與角色對應建議。
* 建立`<product>`的購買群組範本。
* 為不含`<persona>`角色的`<product>`建立購買群組範本，並移除`<role>`角色。
...
| 人物誌 | 角色代表帳戶中互動的人員型別。 代理程式會建立這些資訊，方法是檢視職稱、職能和資歷層級，然後將該資訊標準化，以便在不同帳戶間保持一致。 如此一來，您就能快速瞭解自己是否觸及決策者、影響者或支援角色，而不至於迷失在亂七八糟的標題中。 角色可協助您瞭解哪些人感興趣，而不只是瞭解他們有多大興趣。 |
| 對應購買群組角色 | 將角色對應至角色後，您就可以將角色集合至購買群組：帳戶內最有可能影響或決定購買的完整團隊。 每個角色（例如&#x200B;_決策者_、_影響者_&#x200B;或&#x200B;_冠軍_）都會新增一張圖片，讓您清楚瞭解推動交易的委員會。<br/><br/>代理程式會根據職稱、功能、資歷和您設定的其他屬性，將每個已識別的角色對應到他們最有可能為特定產品扮演的角色。 它也會顯示每個角色的涵蓋範圍，讓您能夠瞭解哪些角色已適當呈現，以及您的參與策略中仍存在哪些差距。 |
...
| 意圖 | 當某人瀏覽網頁或按一下與產品相關的電子郵件連結時，即表示他們感興趣，稱為&#x200B;_意圖_。<br/><br/>代理程式會從分類法開始，分類法基本上是客戶的產品清單以及描述這些產品的關鍵字。 此資訊可協助代理程式瞭解每個內容或互動的內容。<br/><br/>接著，代理程式會使用該分類來標示訪客活動，例如其動作相關的關鍵字或產品。<br/><br/>接著，代理程式會檢視某個人參與的程度，例如造訪多少頁面或互動的頻率。 它會使用此資訊來計算特定關鍵字、產品或產品類別的個別意圖分數。 它將每個意圖分數儲存為&#x200B;_高_、_Medium_&#x200B;或&#x200B;_低_&#x200B;意圖以表示興趣強度。 （低意圖： `<=0.2`，Medium意圖： `0.2 < score <= 0.6`，高意圖： `0.6 < score <= 1`）<br/><br/>最後，代理程式會結合來自相同公司（帳戶）之所有人員的意圖分數，以檢視整體帳戶層級意圖，以顯示公司似乎最感興趣的產品或主題。 |
...
| 角色或角色涵蓋範圍 | 對於任何特定產品，通常都有一組已知會影響購買的關鍵角色和角色（_N_&#x200B;個角色或角色）。<br/><br/>對於每一個帳戶，代理程式會透過檢查該帳戶中至少有一個人員代表其中的&#x200B;_N_&#x200B;角色數目，來計算涵蓋範圍。<br/><br/>如果所有&#x200B;_N_&#x200B;角色都存在，則帳戶具有完整涵蓋範圍。 如果只顯示部分角色，則涵蓋範圍是部分的。<br/><br/>簡單來說，角色和角色涵蓋範圍會根據是否包含所有重要的決策者、影響者和擁護者，來衡量購買群組對產品的完成程度。 |
...
<table>
  <tbody>
    ...
    <tr>
      ...
      <td>用於從Marketo Engage資料中心擷取電子郵件內容；包含(SourceID@SourceInstanceID.SourceType)</td>
    </tr>
    <tr>
      <td>
        <p>
      ...
      <td>僅供參考/記帳；來源型別或來源來源來源來源來源位置的資訊 </td>
    </tr>
    <tr>
      <td>
        <p>
    ...
    <tr>
      ...
      <td>僅供參考/簿記；電子郵件目標來源執行個體的相關資訊</td>
    </tr>
    <tr class="">
      <td>
        <p>
      ...
      <td>僅供參考/記帳；由(SourceID@SourceInstanceID.SourceType)組成</td>
    </tr>
    <tr>
      <td>
        <p>
