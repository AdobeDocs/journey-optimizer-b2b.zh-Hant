...
* _此電子郵件的目標是開始交談並建立可信度。_ 草擬一份語氣不超過120字的電子郵件，並具備諮詢性和同理心。 請務必避免使用過於熟悉或銷售的方法，並且不要使用「希望您一切順利」、「登記就到」或「請」等詞語。_
...
展開電子郵件接觸點以讀取其提示。 本說明在撰寫每個潛在客戶的電子郵件時指導AI，包括語調、長度、焦點和call to action。
...
* **在產生潛在客戶之前完成接觸點提示。**&#x200B;**大量產生之後，通常一次只對一個潛在客戶進行變更。
* **使用推理作為品質檢查。** 如果強調錯誤的訊號（或明顯遺漏訊號），請編輯電子郵件或重新造訪接觸點提示並重新產生步調。
* **將編輯工具與變更相符。**&#x200B;**直接編輯文字與語調；**&#x200B;[!UICONTROL 使用AI產生]**&#x200B;以重新調整或重新組框。
* **僅核准您已檢閱的專案。**&#x200B;**展開接觸點、閱讀內容，並在註冊前調整需要的位置。
...
>[!IMPORTANT]
>
>存取銷售限定詞中的整合需要`Sales Qualifier Admins`使用者群組的成員資格。
