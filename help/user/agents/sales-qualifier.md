...
Account Qualification Agent (AQA)是銷售限定詞的核心。 AQA會使用AI來讀取您的帳戶，並判斷哪些帳戶已準備好進行下一步。 當您的組織已連線CRM （唯讀）時，它可協助研究、電子郵件草擬和CRM資訊內容。

![AI支援的Account Qualification Agent儀表板，用於銷售前景和帳戶研究](./assets/acc-qualification-agent.png){width="800" zoomable="yes"}
...
* **草稿電子郵件**
  ...
  透過綜合來自潛在客戶與帳戶見解的研究來產生電子郵件草稿，以根據BDR目標產生相關的個人化單一電子郵件內容。
...
* _從盈利電話或報告等內容草擬後續電子郵件繪圖。 最多120個字。 主旨列：吸引人，包含關鍵主題。 介紹：使用上下文來源中的直接引號進行連結。 內文：與痛點和價值主張連結。 CTA：建議撥打短電話以進一步探索。_

* _此電子郵件的目標是開始交談並建立可信度。 草擬一份語氣不超過120字的電子郵件，並具備諮詢性和同理心。 請務必避免過於熟悉或銷售方式，不要使用「希望您健康」、「登記使用」或「請」等詞語。_
...
在左側導覽中選取&#x200B;**[!UICONTROL 潛在客戶]**，以檢視您可以存取的所有潛在客戶清單。 它提供事物的快速檢查，例如潛在客戶狀態和上次活動。
...
工作流程建立是五個步驟的精靈： **目標**、**鎖定目標**、**產生接觸點**、**設定**&#x200B;以及&#x200B;**新增潛在客戶**。 每個步驟都建立在最後一步之上，而您定義的目標是首先影響之後的每個決策。
...
目標定位篩選器會定義哪些潛在客戶符合資格。 當您稍後新增潛在客戶時，只有符合這些篩選器的專案才會出現在選取專案清單中。
...
展開電子郵件接觸點以讀取其提示，這是AI在撰寫每個潛在客戶的實際電子郵件（色調、長度、焦點和call to action）時遵循的指示。
...
針對每個產生的電子郵件，**[!UICONTROL 推理]**&#x200B;說明AI如何製作該訊息，包括塑造內容和call to action的訊號、屬性和來源。 在您核准之前，請先檢閱此資訊以驗證個人化。
...
核准的電子郵件佇列會在工作流程&#x200B;**傳送視窗**&#x200B;期間，在設定的&#x200B;**時區**&#x200B;中，在每個接觸點相對於註冊的排定日期傳送。 在您採取行動之前，您未核准的潛在客戶仍會留在&#x200B;**[!UICONTROL 準備檢閱]**&#x200B;中。 核准後，工作流程將根據您定義的步調執行。
...
銷售限定詞中的&#x200B;_任務_&#x200B;區域為業務開發代表(BDR)提供專屬的空間，以管理和處理其對外工作流程動作。 工作會由傳出工作流程引擎自動產生，並代表BDR針對每個潛在客戶所需採取的特定動作，例如電話通話、LinkedIn InMails和電子郵件稽核。

工作管理體驗是設計為&#x200B;**處理佇列**，而不只是待辦事項清單。 您可以開啟工作、執行動作、標籤完成，以及移至下一個工作，這一切都不需要離開頁面。

在左側導覽列中選取&#x200B;**[!UICONTROL 工作]**&#x200B;以開啟完整的[工作]頁面。 這是逐一處理任務的主要工作區。
...
**電子郵件檢閱** — 在系統完成為已註冊工作流程的潛在客戶產生個人化電子郵件時建立。 您會在該潛在客戶開始出站之前檢閱及核准電子郵件。 每個潛在客戶都會取得個別的「電子郵件檢閱」任務；如果您在工作流程中註冊10個潛在客戶，則當產生完成時，您最多會看到10個「電子郵件檢閱」任務。
...
* **[!UICONTROL 標籤完成]** — 主要動作。 在您執行工作後使用此選項 — 進行呼叫、傳送InMail，或檢閱並核准電子郵件。 完成時，工作會記錄為&#x200B;**已完成**，而佇列會自動前進。

* **[!UICONTROL 略過接觸點]** — 可從工作面板的溢位功能表取得。 當您無法完成此特定步驟，但潛在客戶仍然是有效的目標且應保留在工作流程中時，請使用此選項。

  當您略過接觸點時：
   * 潛在客戶會前進到序列中的下一個步驟。 未來的工作仍會依排程產生。
   * 您必須選取原因： *錯誤的連絡資訊*、*錯誤的時間*、*不相關的內容*&#x200B;或&#x200B;*其他* （使用自由文字欄位）。
   * 工作狀態已設定為&#x200B;**已略過**，且已記錄原因和時間戳記。
   * 如果這是工作流程中的最後一個步驟，則潛在客戶的工作流程執行結束。 任務仍記錄為「已略過（未移除）」。

* **[!UICONTROL 從工作流程移除]** — 可從工作面板的溢位功能表取得。 當潛在客戶完全不應再處於此工作流程中時，請使用此選項。

  從工作流程移除潛在客戶時：
   * 此工作流程中該潛在客戶的所有擱置和未來任務都會被取消。
   * 潛在客戶的註冊狀態變更為&#x200B;**已由BDR**&#x200B;移除。
   * 您必須選取原因： *離開公司*、*重複*、*大小錯誤*、*已轉換*&#x200B;或&#x200B;*其他* （含文字欄位）。
   * 出現確認對話方塊： *&quot;這將取消[工作流程名稱]中[潛在客戶]的所有剩餘接觸點。 繼續？&quot;*
   * 工作狀態已設定為&#x200B;**已移除**。 所有已取消的同層級工作也標籤為&#x200B;**已移除**。

>[!NOTE]
>
>略過和移除分析中的原因資料摘要 — 依管道的略過率、依工作流程的移除率，以及主要原因。 這有助於改善工作流程品質，並隨時間通知效能分析。
...
* **逾期** — 到期日已過，且仍未完成的工作。 請先解決這些問題。

* **近期** — 具有未來到期日的任務，其中已完成先前的工作流程步驟。 這些訊息會及早顯示，因此您可以提前計畫，或在時機正確時儘早採取行動（例如，如果您已在與潛在客戶通話）。 會顯示排程的到期日，讓您知道預定時間。
...
即使下一個步驟到期日仍在未來，仍會在潛在客戶完成工作流程步驟時建立近期任務。 這讓您能夠及早瞭解您的管道，以便您可以提前規劃或在機會出現時及早採取行動。
...
>[!IMPORTANT]
>
>存取銷售限定詞中的整合功能需要`Sales Qualifier Admins`使用者群組的成員資格。
...
在Salesforce中，[建立已啟用OAuth且允許API存取身分和資料的外部使用者端應用程式](https://help.salesforce.com/s/articleView?id=xcloud.create_a_local_external_client_app.htm) （或&#x200B;_連線應用程式_），此範圍需遵循您組織的安全性標準。 整合的使用者（例如使用使用者端認證樣式設定時）必須擁有物件的讀取存取權，例如銷售機會、帳戶、連絡人、工作、事件、商機以及相關的商機物件。 管理工作通常需要具有&#x200B;**[!UICONTROL 管理連線應用程式]** （及其他許可權）的使用者在建立後檢視消費者金鑰和密碼。

>[!PREREQUISITES]
>
>若要建立外部使用者端應用程式，您應該成為系統管理員，並確認您已啟用下列專案（從設定檔或許可權集）：
>
>* 自訂應用程式
>* 檢視設定
>* 修改所有資料
...
1. 在左側導覽列中，展開&#x200B;**[!UICONTROL 管理]**&#x200B;並選取&#x200B;**[!UICONTROL 整合]**。
   ...
   您應該會看到Salesforce和Microsoft Dynamics的卡片。
