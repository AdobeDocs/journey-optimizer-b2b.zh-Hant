...
>title="規則類型"
>abstract="為您的管道規則集選取所需的規則類型：使用&#x200B;**頻率上限**&#x200B;類型，將上限規則套用至通訊管道。 例如，每天請勿傳送超過一封電子郵件或簡訊通訊。 選取「**暫停更新**」，定義以時間為基礎的排除方式，確保在特定時段內不會傳送任何訊息。"

>[!CONTEXTUALHELP]
>id="ajo-b2b-prime_rule_sets_duration"
>title="重設上限頻率"
>abstract="選取用來重設上限計數器的行事曆期間：每小時、每日、每週或每月。 計數器會在每個新期間開始時自動重設為0。"
...
依頻道和通訊型別設定頻率上限，以限制設定檔收到多少訊息，並避免客戶因類似通訊而不知所措。 管道規則集會將上限規則套用至通訊管道。 例如，每天請勿傳送超過一封電子郵件或簡訊通訊。
...
### 建立管道頻率上限規則 {#create-capping-rule}
