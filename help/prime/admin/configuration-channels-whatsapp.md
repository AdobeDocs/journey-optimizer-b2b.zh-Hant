...
Journey Optimizer B2B Prime會透過Meta的Cloud API傳送WhatsApp訊息。 在行銷人員建立個人歷程的WhatsApp訊息之前，產品管理員必須設定WhatsApp頻道。

適用於Journey Optimizer B2B Prime的![WhatsApp工作流程](../../user/admin/assets/whatsapp-flow-diagram.png)
...
>[!IMPORTANT]
>
>您使用WhatsApp訊息服務時，必須遵守Meta的條款與條件。 透過Journey Optimizer B2B Prime存取WhatsApp傳訊，即表示您確認已檢閱並同意遵守[Meta WhatsApp商業政策](https://www.whatsapp.com/legal/business-policy/)。

## 限制 {#limitations}

下列限制適用於 WhatsApp 頻道：

* Adobe Journey Optimizer B2B Prime **不符合HIPAA標準，且不符合HIPAA標準**。 此外，Adobe的BAA不包含第三方廠商。 客戶需自行負責法規遵循及供應商驗證。
...
在傳送WhatsApp訊息之前，您必須先設定Journey Optimizer B2B Prime環境，並將其與您的WhatsApp帳戶連結。
...
1. 設定您的API認證，如下所述：
   ...
   * **[!UICONTROL 企業帳戶ID]** — 輸入與您的企業組合相關的唯一編號。 如需詳細資訊，請參閱[Meta檔案](https://www.facebook.com/business/help/1181250022022158?id=180505742745347)。
   ...
   ![WhatsApp設定API認證](../../user/admin/assets/config-whatsapp-channels-api-credentials.png){width="500" zoomable="yes"}

1. 按一下&#x200B;**[!UICONTROL 繼續]**。

1. 選擇您要連線至您的WhatsApp API認證的&#x200B;**[!UICONTROL WhatsApp商業帳戶]**。

   ![WhatsApp設定API認證選取商業帳戶](../../user/admin/assets/config-whatsapp-channels-api-credentials-details.png){width="500" zoomable="yes"}
...
1. 從外部測試認證 — 直接使用Meta API驗證您的認證，以確認問題與認證有關，或與Journey Optimizer B2B Prime認證處理有關。
...
Webhook可讓Journey Optimizer B2B Prime接收來自WhatsApp商業帳戶的傳入訊息、同意回應和傳送通知。 設定Webhook以確保適當的同意管理和訊息追蹤。
...
1. 輸入&#x200B;**[!UICONTROL 回複訊息]**，當收到的訊息符合此類別的關鍵字時，自動傳送。
   ...
   ![WhatsApp設定Webhook包含選擇加入關鍵字和回複訊息](../../user/admin/assets/config-whatsapp-channels-webhooks.png){width="500" zoomable="yes"}
...
1. 複製&#x200B;**[!UICONTROL 驗證Token]**&#x200B;和&#x200B;**[!UICONTROL Webhook URL]**&#x200B;值。
   ...
   ![WhatsApp設定webhook設定以複製URL及驗證Token](../../user/admin/assets/config-whatsapp-channels-webhooks-copy-token-url.png){width="500" zoomable="yes"}
...
1. 輸入用於郵件傳遞的&#x200B;**[!UICONTROL 寄件者電話號碼]**。
   ...
   ![WhatsApp通道設定詳細資料](../../user/admin/assets/config-whatsapp-channels-general-create.png){width="500" zoomable="yes"}

1. （目前不適用於Journey Optimizer B2B Prime）針對&#x200B;**[!UICONTROL WhatsApp執行欄位]**，選取當收件者有多個電話號碼可用時，要作為優先電話號碼使用的設定檔屬性。
