...
布建完成並繫結沙箱後，請完成下列步驟，為您的團隊和使用者提供[!DNL Journey Optimizer B2B Prime]存取權。
...
>[!IMPORTANT]
>
>[!DNL Journey Optimizer B2B Prime]存取權需要您啟用使用下列命名慣例布建的特定沙箱： Marketo Engage訂閱首碼+ Prime。 例如，如果連結的Marketo Engage訂閱首碼為&#x200B;_AcmeAssoc_，則存取[!DNL Journey Optimizer B2B Prime]所需的沙箱為&#x200B;_AcmeAssocPrime_。
...
1. 選取為[!DNL Journey Optimizer B2B Prime]存取權布建的沙箱(`<Marketo subscription prefix>Prime`)。
...
您的自訂角色已設定，而且指派群組中的使用者現在可以存取您選取的[!DNL Journey Optimizer B2B Prime]權能。
