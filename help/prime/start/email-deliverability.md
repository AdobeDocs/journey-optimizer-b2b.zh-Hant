...
[!DNL Adobe Journey Optimizer B2B Prime]中的電子郵件傳遞能力是一組基礎架構和驗證設定，可協助電子郵件訊息到達收件者的收件匣，而非垃圾郵件資料夾，且不會被ISP （網際網路服務提供者）封鎖。
...
![Journey Optimizer B2B Prime的電子郵件傳遞能力設定](./assets/email-deliverability-diagram.svg){width="550" zoomable="yes"}
...
| ------- | ---------------------- |
| **_子網域_** | 您用來透過[!DNL Journey Optimizer B2B Prime]傳送電子郵件的傳送網域委派部分（例如，`mail.contoso.com`）。 子網域會將您的B2B行銷信譽與公司或交易郵件隔離。 |
| **_IP集區_** | 與一或多個子網域相關聯的一組IP位址。 [!DNL Journey Optimizer B2B Prime]在此版本中支援Adobe管理的共用IP集區；專用IP集區位於GA藍圖上。 |
...
>[!NOTE]
>
>[!DNL Journey Optimizer B2B Prime]中的每個子網域只能由一個Adobe產品使用。 您無法在[!DNL Journey Optimizer B2B Prime]和其他產品（例如Adobe Marketo Engage或Adobe Campaign）之間共用相同的傳送子網域，您必須使用不同的子網域。

### 支援的方法 {#supported-methods}

[!DNL Journey Optimizer B2B Prime]支援此Beta發行版本中三種子網域委派方法中的兩種。 第三個方法（自訂委派）在藍圖上。
...
>* 確認您擁有[!DNL Journey Optimizer B2B Prime]中的管理員角色。

1. 在[!DNL Journey Optimizer B2B Prime]左側導覽中，展開&#x200B;**[!UICONTROL 管理]**&#x200B;並選取&#x200B;**[!UICONTROL 管道]**。
...
1. 當記錄已就緒並傳播時，請返回[!DNL Journey Optimizer B2B Prime]並確認。
1. 按一下&#x200B;**[!UICONTROL 提交]**。
...
[!DNL Journey Optimizer B2B Prime]會產生您子網域的確切CNAME和TXT記錄，並讓您將其下載為CSV檔案。 使用以下提供者專屬步驟，協助您的DNS團隊找到正確的設定畫面並新增每筆記錄。
...
>[!IMPORTANT]
>
>您為[!DNL Journey Optimizer B2B Prime]新增的每個記錄都必須顯示灰色雲端（僅限DNS），而非橘色雲端（已代理）。 代理記錄會透過Cloudflare的伺服器而非Adobe路由流量，這會中斷DKIM簽署、點選追蹤和跳出處理。 如果記錄顯示橘色，請按一下雲端圖示將其切換為灰色。
...
>[!NOTE]
>
>使用標準CNAME記錄集，而不是「別名記錄集」選項，此選項僅指向Azure資源，而不是外部主機名稱。 每個CNAME記錄集只保留一個目標，符合[!DNL Journey Optimizer B2B Prime]發出記錄的方式 — 每個主機一個CNAME。
...
* **子網域重複使用：**&#x200B;其他Adobe產品(Marketo Engage、Adobe Campaign)已使用的子網域無法在[!DNL Journey Optimizer B2B Prime]中重複使用。
